 ...
 */
function getFeatureGood(call, callback) {
  callback(null, checkFeature(call.request));
}

function getFeatureBad(call, callback) {
  callback(null, checkFeature(call.request));
}
...
function getServer() {
  ...
  server.addService(services.RouteGuideService, {
    getFeatureGood: getFeatureGood,
    getFeatureBad: (call, callback) => {
      // console.log(call.call);
      // console.log(server.sessions)
      // console.log(server.http2ServerList[0].close)
      const http2Server = Array.from(server.http2ServerList)[0];
      // http2Server.close();

      const http2Session = Array.from(server.sessions)[0];
      http2Session.goaway(0x07)
      // http2Session.destroy(new Error('this is destructive'), 7);
      // http2Session.close()
      // const serverHttp2Stream = call.call;

      // serverHttp2Stream.sendError({
      //   code: 14,
      //   details: 'test error',
      //   metadata: new Metadata()
      // })
      // serverHttp2Stream.emit('cancelled', 'cancelled');
      // serverHttp2Stream.emit('streamEnd', false);
      // serverHttp2Stream.sendStatus({code: Status.CANCELLED, details: 'Cancelled by client', metadata: new Metadata()});

      // stream.sendStatus({code: Status.UNAVAILABLE, details: 'Unavailable', metadata: new Metadata()})
      // getFeatureBad(call, callback)
      // http2Session.destroyed = true;
      callback(null, checkFeature(call.request));
    },
  });
  return server;
